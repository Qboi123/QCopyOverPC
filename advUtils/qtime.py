...
class QTimePeriod(object):
    ...
    def get_start_time(self):
        ...
        if _time is not None:
            ...
            days = int(days)
        if self.period == 0:
            return QTime(days, 0, 0, 0)
        if self.period == 1:
            return QTime(days, 6, 0, 0)
        if self.period == 2:
            return QTime(days, 12, 0, 0)
        if self.period == 3:
            return QTime(days, 18, 0, 0)
    ...
    def get_end_time(self):
        ...
        if _time is not None:
            ...
            days = int(days)
        if self.period == 0:
            return QTime(days, 5, 59, 59)
        if self.period == 1:
            return QTime(days, 11, 59, 59)
        if self.period == 2:
            return QTime(days, 17, 59, 59)
        if self.period == 3:
            return QTime(days, 23, 59, 59)
    
    def get_timespan(self):
        return QTimeSpan(self.get_start_time(), self.get_end_time())
    ...
    def system_timeperiod(cls):
        return QTime.system_time().get_timeperiod()


class QTimeLength(object):
    def __init__(self, time: float):
        self._timeLength = time
        
    def sleep(self):
        _time.sleep(self._timeLength)
    ...
    def __str__(self):
        g = _time.gmtime(self._timeLength)
        minute = g.tm_min
        second = g.tm_sec
...
class QTimeSpan(object):
    def __init__(self, time_start, time_end):
        if type(time_start) == int:
            self._start: QTime = QTime(time_start)
        elif type(time_start) == float:
            self._start: QTime = QTime(time_start)
        elif type(time_start) == QTime:
            self._start: QTime = time_start

        if type(time_end) == int:
            self._end: QTime = QTime(time_end)
        elif type(time_end) == float:
            self._end: QTime = QTime(time_end)
        elif type(time_end) == QTime:
            self._end: QTime = time_end
    ...
    def get_timelength(self):
        return QTimeLength(self._end.seconds - self._start.seconds)
...
class QTime(object):
    @overload
    def __init__(self, seconds: Union[int, float, _struct_time, tuple]):
        if type(seconds) == int:
            ...
            try:
                ...
                try:
                    ...
                    raise TypeError(f"Invalid type for argument 'seconds': '{type(seconds)}' must be float or integer.")
                except ValueError:
                    raise TypeError(f"Invalid value for argument 'seconds': {repr(seconds)}.")
            except ValueError:
                try:
                    self.seconds = float(seconds)
                except TypeError:
    ...
    @__init__.add
    def __init__(self, timetuple: Union[Tuple, _struct_time]):
        if type(timetuple) == _struct_time:
            self.timetuple = _time.mktime(timetuple)
        elif type(timetuple) == tuple:
            self.timetuple = _time.mktime(timetuple)
        else:
            try:
                self.seconds = _time.mktime(timetuple)
            except TypeError:
                raise TypeError(f"invalid type for argument #0: '{type(timetuple)}' must be tuple or structtime")
    ...
    def __init__(self, day, month, year, hour: int, minute: int, second: int):
        self.seconds = _time.mktime(_struct_time((year, month, day, hour, minute, second, -1, -1, -1)))
    ...
    def asctime(self):
        return _time.asctime(self.gmtime())

    def gmtime(self):
        return _time.gmtime(self.seconds)

    def sleep(self):
        return _time.sleep(self.seconds)

    def mktime(self):
        return _time.mktime(self.gmtime())
    ...
    def get_timeperiod(self):
        ...
        hour = self.get_hour()
        if 0 <= hour < 6:
            return QTimePeriod(0, self)
        elif 6 <= hour < 12:
            return QTimePeriod(1, self)
        elif 12 <= hour < 18:
            return QTimePeriod(2, self)
        elif 18 <= hour < 24:
            return QTimePeriod(3, self)

    def get_weekday(self):
        return QWeekDay(self.get_wday())
    ...
    def strftime(self, format_):
        ...
        return _time.strftime(format_, self.gmtime())


NIGHT = QTimePeriod(0)
MORNING = QTimePeriod(1)
NOON = QTimePeriod(2)
AFTERNOON = QTimePeriod(3)
...
if __name__ == '__main__':
    def time_tests():
        ...
        time_ = QTime(5, 7, 6, 42)
        print(time_.get_timeperiod())

        time_ = QTime(5, 14, 38, 32)
        print(time_.get_timeperiod())

        time_ = QTime(5, 19, 54, 23)
        print(time_.get_timeperiod())

        time_ = QTime(4, 3, 2004, 21, 25, 0)
        print(time_)

        time_ = QTime.system_time()
        print(time_.get_timeperiod())
    ...
    def timespan_tests():
        ...
        time1 = QTime(17000, 13, 59, 32)
        time2 = QTime(18300, 17, 32, 19)
        time_s = QTimeSpan(time1, time2)
