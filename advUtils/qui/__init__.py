...
from advUtils import QApplication
...
class QSceneManager(object):
    def __init__(self, root: _tk.Tk):
        self._scenes: _t.Dict[str, 'QScene'] = {}
        self._root = root
        self.currentScene: _t.Optional[QScene] = None
        self.currentSceneName: _t.Optional[str] = None

    def add_scene(self, prefix: str, name: str, scene: _t.Type['QScene']) -> str:
        """
        Adds a new scene. Prefix and name should not contain spaces or colons.

        :param prefix: The prefix for the name to use.
        :param name: The name to use.
        :param scene: The Scene object to add.
        :returns: The namespace ID of the scene.
        :raises ValueError: If the name or prefix contains a colon
        """

        notallowedpattern = re.compile("(^[a-zA-Z0-9_]+$)")
        if notallowedpattern.match(name) is None:
            raise ValueError(f"invalid scene name: {repr(name)}, special characters are disallowed.")
        if notallowedpattern.match(prefix) is None:
            raise ValueError(f"invalid scene prefix: {repr(name)}, special characters are disallowed.")

        self._scenes[prefix + ":" + name] = scene(self._root, self)
        return prefix + ":" + name
    ...
    def change_scene(self, name, *args, **kwargs):
        ...
        new_scene = self.get_scene(name)
        self.currentSceneName = name
        self.currentScene: QScene = new_scene
        self.currentScene.show_scene(*args, **kwargs)

    def scene_exists(self, name):
        return name in self._scenes.keys()
...
class QScene(object):
    def __init__(self, root, scenemanager: QSceneManager):
        self.frame = _ttk.Frame(root)
...
class QCanvasScene(QScene):
    def __init__(self, root, scenemanager: QSceneManager):
        super(QCanvasScene, self).__init__(root, scenemanager)
        self.canvas = _tk.Canvas(self.frame, highlightthickness=0)
        self.canvas.pack(fill="both", expand=True)

    def __repr__(self):
        return super(QCanvasScene, self).__repr__()
...
class QSpacing(_ttk.Frame):
    def __init__(self, master, **kw):
        super(QSpacing, self).__init__(master, **kw)
...
class QWindow(QFrame):
    def __init__(self, application: QApplication, killonclose: bool = True):
        self._root = _tk.Tk()

        if killonclose:
            self._root.wm_protocol("WM_DELETE_WINDOW", lambda: application.kill(0))

        super(QWindow, self).__init__(self._root)

        self.__fullscreen = False
        self.__alpha = 1.0
        self.fullscreen = property(lambda: self.__fullscreen, self.set_fullscreen)
        self.alpha = property(lambda: self.__alpha, self.set_alpha)

    def set_title(self, title):
        self._root.wm_title(title)

    def get_title(self):
        return self._root.wm_title()

    def set_fullscreen(self, enabled=None):
        if enabled is None:
            self.set_fullscreen(not self["fullscreen"])

        self._root.wm_attributes("-fullscreen", enabled)
        self.__fullscreen = enabled
...
if __name__ == '__main__':
    ...
    def test_regex():
        notallowedpattern = re.compile("(^[a-zA-Z0-9_]+$)")
        print(notallowedpattern.match("hallo"))
        print(notallowedpattern.match("hallo:3"))

    def test_scenemanager():
        root = _tk.Tk()
        root.wm_minsize(200, 150)
        init(root)

        scenemgr = QSceneManager(root)
        scenemgr.add_scene("qui", "testscene", QScene)
        scenemgr.add_scene("qui", "testcanvasscene", QCanvasScene)

        _ttk.Label(scenemgr.get_scene("qui:testscene").frame, text="1: testscene [Scene]").pack()
        # noinspection PyUnresolvedReferences
        scenemgr.get_scene("qui:testcanvasscene").canvas.create_text(10, 10, text="2: testcanvasscene [CanvasScene]",
                                                                     anchor="nw")

        print("Select a scene to open:")
        print("  1: testscene [Scene]")
        print("  2: testcanvasscene [CanvasScene]")
        print("")
        ...
        if inp == "1":
            scenemgr.change_scene("qui:testscene")
        if inp == "2":
            scenemgr.change_scene("qui:testcanvasscene")
